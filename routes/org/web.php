...
Route::prefix('{org_slug}')->group(function () {
	Route::get('/', function ($org_slug) {
		if (Auth::check() && !empty(config('bookdose.app.main_product_redirect')))
			return redirect(config('bookdose.app.main_product_redirect').'/'.$org_slug);
		else
			return view('welcome');
	})->middleware('auth');

	Route::get('/home', function ($org_slug) {
		// dd(config('bookdose.app.main_product_redirect').'/'.$org_slug);
		if (Auth::check() && !empty(config('bookdose.app.main_product_redirect')))
			return redirect(config('bookdose.app.main_product_redirect').'/'.$org_slug);
		else
			return view('welcome');
