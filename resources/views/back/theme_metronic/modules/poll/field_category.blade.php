	...
	@forelse ($categories as $c)
		<div class="col-3">
			<div class="kt-checkbox-inline">
				<label class="kt-checkbox kt-checkbox--brand kt-checkbox--{{ ($c->id == ($selected_id ?? '') || in_array($c->id, ($poll->selected_categories ?? []))) ? 'solid' : 'bold' }}">
					 <input type="checkbox" name="poll_categories[]" value="{{ $c->id }}" {{ ($c->id == ($selected_id ?? '') || in_array($c->id, ($poll->selected_categories ?? []))) ? 'checked' : '' }}> {{ $c->title }}
					 <span></span>
				</label>
			 </div>
		</div>
