...
@section('title', 'Poll Category')
@section('page_title', 'Poll Category')

@section('topbar_button')
	<a href="{{ route('admin.poll.category.index') }}" class="btn btn-label-brand btn-bold">
		<i class="fa fa-arrow-left"></i> Back
	</a>
	@isset ($category)
	<a href="{{ route('admin.poll.category.create') }}" class="btn btn-outline-brand btn-bold">
...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
	<div class="kt-portlet">
		<div class="kt-portlet__head">
			...
			<div class="kt-portlet__head-toolbar">
				<div class="kt-form__actions">
					 ...
					 <?php } ?>
					 <a href="{{ route('admin.poll.category.index') }}" class="ml-1 btn btn-secondary btn-bold btn-wide kt-font-transform-u">Cancel</a>
				</div>
			</div>
		</div>
		<div class="kt-portlet__body kt-portlet__body--fit">
			...
			<form id="frm_main" class="kt-form" action="{{ isset($category) ? route('admin.poll.category.update', $category->id) : route('admin.poll.category.store') }}" method="POST" enctype="multipart/form-data">
				@csrf
				...
				<div class="kt-portlet__body">
					<div class="kt-section mb-0">
						...
						<div class="form-group">
							<label><sup><i class="la la-asterisk fs-10 text-danger"></i></sup> ชื่อหมวดหมู่โพล:</label>
							<input id="title" name="title" type="text" class="form-control required" placeholder="Enter poll title" value="{{ $category->title ?? old('title') }}" autocomplete="off">
						</div>
